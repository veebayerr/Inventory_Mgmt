<?php
	...
	if(isset($_POST['Submit'])){
		...
		$sql = "SELECT sum(payment_ammount) FROM `payment` WHERE payment_date >= '{$_POST['start_date']}' and payment_date <= '{$_POST['end_date']}' and store_no = {$storeNo} and payment_type = 'cash'";
		$cash = $conn->query($sql);
		$sql = "SELECT sum(payment_ammount) FROM `payment` WHERE payment_date >= '{$_POST['start_date']}' and payment_date <= '{$_POST['end_date']}' and store_no = {$storeNo} and payment_type = 'credit'";
		$credit = $conn->query($sql);
		$sql = "SELECT sum(payment_ammount) FROM `payment` WHERE payment_date >= '{$_POST['start_date']}' and payment_date <= '{$_POST['end_date']}' and store_no = {$storeNo} and payment_type = 'check'";
		$check = $conn->query($sql);
		$cashval = 0;
		$paid_total = 0;
		$sql = "SELECT sum(`price_before_tax`) FROM `transaction` WHERE `store_no` ='{$storeNo}' and `transaction_date` <='{$_POST['end_date']}' and `transaction_date` >='{$_POST['start_date']}'";
